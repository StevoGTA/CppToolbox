...
class CUUIDInternals{
	public:
						CUUIDInternals() : mUUIDBytes(eCreateUUIDBytes()), mReferenceCount(1) {}
						CUUIDInternals(const SUUIDBytes& uuidBytes) : mUUIDBytes(uuidBytes), mReferenceCount(1) {}
						CUUIDInternals(const CData& data) : mReferenceCount(1)
							{
						...
						CUUIDInternals(const CString& string) : mReferenceCount(1)
							{
								// Check length
		...
		CUUIDInternals*	addReference() { mReferenceCount++; return this; }
		void			removeReference()
							{
								// Decrement reference count and check if we are the last one
								if (--mReferenceCount == 0) {
									// We going away
									CUUIDInternals*	THIS = this;
									DisposeOf(THIS);
								}
							}
