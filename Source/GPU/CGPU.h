...
class CGPU {
	...
	public:
													...
													// Instance methods
		CVideoCodec::DecodeFrameInfo::Compatibility	getVideoCodecDecodeFrameInfoCompatibility() const;

		I<CGPUTexture>								registerTexture(const CData& data,
		...
		void										renderStart(const S2DSizeF32& size2D,
															Float32 fieldOfViewAngle3D = 0.0f,
															Float32 aspectRatio3D = 1.0f, Float32 nearZ3D = 0.01f,
															Float32 farZ3D = 100.0f,
															const S3DPointF32& camera3D = S3DPointF32(),
															const S3DPointF32& target3D = S3DPointF32(),
															const S3DVectorF32& up3D = S3DVectorF32()) const;
		void										render(CGPURenderState& renderState, RenderType renderType,
															UInt32 count, UInt32 offset);
		void										renderIndexed(CGPURenderState& renderState, RenderType renderType,
															UInt32 count, UInt32 offset);
