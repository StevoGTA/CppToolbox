...
CString	CAudioPlayer::mPlaybackPositionUpdatedNotificationName(OSSTR("playbackPositionUpdated"));
CString	CAudioPlayer::mPlaybackPositionKey(OSSTR("playbackPosition"));

CString	CAudioPlayer::mEndOfDataNotificationName(OSSTR("endOfData"));
...
CAudioPlayerReaderThread::CAudioPlayerReaderThread(CAudioPlayer& audioPlayer, CSRSWBIPSegmentedQueue& queue,
	...
	CThread(CString(OSSTR("Audio Player ")) + audioPlayer.getIdentifier() + CString(OSSTR(" (Reader)")))
//----------------------------------------------------------------------------------------------------------------------
{
	// Setup
	mInternals =
...
//----------------------------------------------------------------------------------------------------------------------
//----------------------------------------------------------------------------------------------------------------------
// MARK: - CAudioPlayerPlaybackPositionUpdatedNotificationThreadInternals

class CAudioPlayerPlaybackPositionUpdatedNotificationThreadInternals {
	public:
		CAudioPlayerPlaybackPositionUpdatedNotificationThreadInternals(CNotificationCenter& notificationCenter,
				CAudioPlayer& audioPlayer) :
			mNotificationCenter(notificationCenter), mAudioPlayer(audioPlayer), mShutdownRequested(false),
					mQueuePlaybackPositionUpdatedNotification(false), mPlaybackPosition(0.0)
			{}
		...
		bool					mShutdownRequested;
		bool					mQueuePlaybackPositionUpdatedNotification;
		CSemaphore				mSemaphore;
		UniversalTime			mPlaybackPosition;
};
...
//----------------------------------------------------------------------------------------------------------------------
CAudioPlayerPlaybackPositionUpdatedNotificationThread::CAudioPlayerPlaybackPositionUpdatedNotificationThread(
		CNotificationCenter& notificationCenter, CAudioPlayer& audioPlayer) :
	CThread(CString(OSSTR("Audio Player ")) + audioPlayer.getIdentifier() + CString(OSSTR(" (Position)")))
//----------------------------------------------------------------------------------------------------------------------
{
	// Setup
	mInternals = new CAudioPlayerPlaybackPositionUpdatedNotificationThreadInternals(notificationCenter, audioPlayer);
...
//----------------------------------------------------------------------------------------------------------------------
void CAudioPlayerPlaybackPositionUpdatedNotificationThread::run()
//----------------------------------------------------------------------------------------------------------------------
{
	// Run until shutdown
	while (!mInternals->mShutdownRequested) {
		...
		// Check for queueing notification
		if (mInternals->mQueuePlaybackPositionUpdatedNotification) {
			// Queue notification
			CDictionary	info;
			info.set(CAudioPlayer::mPlaybackPositionKey, mInternals->mPlaybackPosition);
			mInternals->mNotificationCenter.queue(CAudioPlayer::mPlaybackPositionUpdatedNotificationName,
					&mInternals->mAudioPlayer, info);

			// Done
			mInternals->mQueuePlaybackPositionUpdatedNotification = false;
		}
	}
}
...
//----------------------------------------------------------------------------------------------------------------------
void CAudioPlayerPlaybackPositionUpdatedNotificationThread::notePlaybackPositionUpdated(
		UniversalTimeInterval timeInterval)
//----------------------------------------------------------------------------------------------------------------------
{
	// Update info
	mInternals->mPlaybackPosition = timeInterval;

	// Signal
	mInternals->mQueuePlaybackPositionUpdatedNotification = true;
	mInternals->mSemaphore.signal();
}

//----------------------------------------------------------------------------------------------------------------------
void CAudioPlayerPlaybackPositionUpdatedNotificationThread::shutdown()
//----------------------------------------------------------------------------------------------------------------------
{
	// Request shutdown
	mInternals->mShutdownRequested = true;
...
//----------------------------------------------------------------------------------------------------------------------
//----------------------------------------------------------------------------------------------------------------------
// MARK: - CAudioPlayerEndOfDataNotificationThreadInternals

class CAudioPlayerEndOfDataNotificationThreadInternals {
	public:
		CAudioPlayerEndOfDataNotificationThreadInternals(CNotificationCenter& notificationCenter,
				CAudioPlayer& audioPlayer) :
			mNotificationCenter(notificationCenter), mAudioPlayer(audioPlayer), mShutdownRequested(false),
					mQueueEndOfDataNotification(false)
			{}
		...
		bool					mShutdownRequested;
		bool					mQueueEndOfDataNotification;
		CSemaphore				mSemaphore;
};
...
//----------------------------------------------------------------------------------------------------------------------
CAudioPlayerEndOfDataNotificationThread::CAudioPlayerEndOfDataNotificationThread(
		CNotificationCenter& notificationCenter,  CAudioPlayer& audioPlayer) :
	CThread(CString(OSSTR("Audio Player ")) + audioPlayer.getIdentifier() + CString(OSSTR(" (End of Data)")))
//----------------------------------------------------------------------------------------------------------------------
{
	// Setup
	mInternals = new CAudioPlayerEndOfDataNotificationThreadInternals(notificationCenter, audioPlayer);
...
//----------------------------------------------------------------------------------------------------------------------
void CAudioPlayerEndOfDataNotificationThread::run()
//----------------------------------------------------------------------------------------------------------------------
{
	// Run until shutdown
	while (!mInternals->mShutdownRequested) {
		...
		// Check for queueing notification
		if (mInternals->mQueueEndOfDataNotification) {
			// Queue notification
			mInternals->mNotificationCenter.queue(CAudioPlayer::mEndOfDataNotificationName, &mInternals->mAudioPlayer);

			// Done
			mInternals->mQueueEndOfDataNotification = false;
		}
	}
}
...
//----------------------------------------------------------------------------------------------------------------------
void CAudioPlayerEndOfDataNotificationThread::noteEndOfData()
//----------------------------------------------------------------------------------------------------------------------
{
	// Signal
	mInternals->mQueueEndOfDataNotification = true;
	mInternals->mSemaphore.signal();
}

//----------------------------------------------------------------------------------------------------------------------
void CAudioPlayerEndOfDataNotificationThread::shutdown()
//----------------------------------------------------------------------------------------------------------------------
{
	// Request shutdown
	mInternals->mShutdownRequested = true;
