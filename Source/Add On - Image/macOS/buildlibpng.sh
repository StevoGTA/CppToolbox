...
function make_lib_for_arch() {
  # reset flags
  ARCH="$1"
  CFLAGS="-Wall -arch $ARCH -funwind-tables -fembed-bitcode -DPNG_ARM_NEON_OPT=0"
  ASMFLAGS="-arch $ARCH"
...
for arch in $ARCHS_TO_BUILD ; do
  ...
  LIPO_ARGS="$LIPO_ARGS -arch $arch build-$arch/libpng.a"
done

$LIPO $LIPO_ARGS -create -output libpng.a
file libpng.a
